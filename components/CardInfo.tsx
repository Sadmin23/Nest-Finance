...
import { items } from '../app/card-comparison/data'
...
const CardInfo = (): JSX.Element => {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/bankapi/card/')
    ...
    <div>
        ...
        <div className="grid grid-cols-3 gap-5 mx-10">
          {currentItems.map((data, index) => (
            <CreditCard key={index} Name={data.network + " " + data.usage + " Card"} image="/brand_assets/ctbank.png" />
          ))}
        </div>
        <div className='mt-6 mx-auto w-60 space-x-4'>
